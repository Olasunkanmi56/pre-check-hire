...
import 'package:precheck_hire/screens/jobseeker_screens/jpbseeker_submit_kyc.dart';
import 'package:precheck_hire/services/auth_service.dart'; // Make sure this exists

class JobSeekerEmailVerificationScreen extends StatefulWidget {
  final String email;

  const JobSeekerEmailVerificationScreen({super.key, required this.email});

  @override
  State<JobSeekerEmailVerificationScreen> createState() =>
      _JobSeekerEmailVerificationScreenState();
}

class _JobSeekerEmailVerificationScreenState
    extends State<JobSeekerEmailVerificationScreen> {
  ...
  void _verifyEmailToken() async {
    ...
    try {
      ...
      await _authService.verifyEmailToken(email: email, token: token);
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (context) => const JobSeekerSubmitKYCScreen(),
        ),
      );
    } catch (e) {
      ScaffoldMessenger.of(
